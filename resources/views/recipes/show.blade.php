            ...
            <div class="mt-5 flex lg:mt-0 lg:ml-4">
                @can('update', $recipe)
                    <x-button href="{{ route('recipes.edit', $recipe) }}">
                        <x-icons.pencil class="-ml-1 mr-2 h-5 w-5 text-gray-500 dark:text-gray-300"/>
                        {{ __('Edit') }}
                    </x-button>
                @endcan
            </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-4 gap-5">
            <div class="lg:col-span-3 grid grid-cols-1 gap-5">
                ...
                <div class="flex flex-col md:flex-row">
                    ...
                    <div class="surface w-full mt-5 md:mt-0 {{ $recipe->description ? 'md:w-1/2': '' }}">
                        <div class="px-4 pt-3 pb-2 text-lg font-bold">
                            {{ __('Ingredients') }}
                        </div>
                        @foreach($recipe->ingredients as $ingredient)
                            <div class="px-4 py-2 text-md border-t border-gray-200 dark:border-gray-600">
                                {{ $ingredient->name }}
                            </div>
                        @endforeach
                    </div>
                ...
                <div class="surface">
                    ...
                    @foreach($recipe->instructions as $instruction)
                        <div class="p-4 text-md border-t border-gray-200 dark:border-gray-600">
            ...
            <div>
                <div class="surface mb-5">
                    <dl>
                        <x-item label="{{ __('Author') }}" class="border-none">
                        ...
                        <x-item label="{{ __('Category') }}">
                            {{ $recipe->category->name }}
                        </x-item>

                        <x-item label="{{ __('Cooking time') }}">
                            {{ \App\Services\DurationConverter::toHuman($recipe->duration) }}
                        </x-item>

                        <x-item label="{{ __('Published at') }}">
